...
class Product(models.Model):
    user = models.ForeignKey(User, on_delete=models.SET_NULL, null=True) # OnetoMany relation with users, we don't wanna delete childre (products) if their patent (user) get deleted
    name = models.CharField(max_length=100, null=True, blank=True)
    image = models.ImageField(null=True, blank=True) # it needs pillow
...
class OrderItem(models.Model):
    ...
    price = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True)
    image = models.CharField(max_length=100, null=True, blank=True) # for image url
    _id = models.AutoField(primary_key=True, editable=False)
